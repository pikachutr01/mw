...
| Bildirimde `icon` ile `badge`e aynı dosyayı vermek | `badge` Android **durum çubuğu** simgesidir ve tarayıcı orada rengi tamamen atıp **yalnız alfa kanalını** maske olarak kullanır. Tam opak bir PNG'nin maskesi dolu bir karedir → durum çubuğunda içi dolu leke | `icon` renkli 192 px (uygulama ikonu), `badge` **monokrom siluet + şeffaf** 96 px (`public/badge-96.png`, üreteci `apps/web/scripts/badge-png.mjs`). iOS `badge`i yok sayar |
