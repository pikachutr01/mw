...
| **Kahraman yaşam döngüsü** | Ölüm yalnız durum %0,0'da. Sağ birlik varsa ölü taşınır, yoksa **yok edilir** (tapınakta 1 sa). XP havuzu **kazanan 2/3 · kaybeden 1/3**, taraf içi `1/(sv+1)` ağırlıkla. Diriltme `9 sa × 0,93^(Tapınak+Sv)` — süre seviyeyle KISALIR, maliyet artar | §13.11.4d |
...
| **Mesajlar** | ✅ Raporlar/Mesajlar sekmeleri · tek tip liste satırı · **zengin savaş raporu modalı** (§7.1b) |
| **Komuta Merkezi** | ✅ Genel Durum + İttifak (§13.15b) + **Arama** (§13.18.0) + Sıralamalar (Oyuncu ✅ · Kahraman ✅ · İttifak ✅) |
| **Seçenekler** | ✅ hesap (e-posta + doğrulama rozeti + **Şifre Değiştir**) + **Şehir paneli** (Şehir Adı Değiştir; orijinalde de bu menüde — `g.java` case 63) + tema (tema **yalnız burada**) + **Bildirimler paneli** (izin düğmesi + 4 kategori anahtarı, §7.2b) |
