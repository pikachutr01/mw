...
| ⭐⭐⭐ Bir kapının **kendisini açacak eylemi de engellemesi** | Kale bütçesi (Σ bina ≤ Kale×10) yükseltmeleri denetlerken **kalenin kendi yükseltmesini de** denetliyordu. Normalde görünmez: kapı zaten `used > total` olmasına izin vermiyor. 2026-08-12 veri göçü (her şehre baraka sv1) tam sınırdaki şehirleri 21/20'ye itince ortaya çıktı — oyuncu HİÇBİR şeyi yükseltemez oldu, **kale dâhil**. Ekrandaki hata *"Kale seviyeniz yetersiz: 21/20. **Kale'yi yükseltin.**"* — mesaj çözümü söylüyor, kod tam da onu yasaklıyor. Bütçeyi artırmanın tek yolu kale olduğu için çıkışsız döngü. ⚠️ Ben bu riski göçten önce fark edip *"Kale 3'e çıkınca kendiliğinden düzelir"* demiştim; **sonucu yanlış öngördüm** — kaleye çıkmanın kendisi kapalıydı | Bir kısıt eklerken sor: *"Bu kısıta takılan oyuncu kurtulmak için ne yapmalı? O eylem de aynı kısıta takılıyor mu?"* Kural düzeltmesi tek cümle: **bütçe TÜKETİCİLERİ kısıtlar, kaynağını değil** — kale zaten `consumesCastleBudget: false`, onu kendi ürettiği bütçeye karşı sınamak kategori hatasıydı. ⚠️ Yalnız `total`ı yeni seviyeden hesaplamak YETMEZ: bütçeyi bir seviyelik kazançtan (10) fazla aşmış şehir yine kilitli kalırdı — kaynak yapının yükseltmesi **koşulsuz** geçmeli |
| ⭐⭐⭐ Bir **korumanın** başka bir kuralın **girdisi** olduğunu görmemek | Hesap silme, kalan şehri *"vitrine çıkmasın"* diye `ranking_excluded` işaretliyordu. Ama 10 kat kuralı puanı **sıralama satırından** okuyor ve satırı olmayanı 0 (kelepçeyle 1) sayıyor → muaf hesaba 10 puandan büyük hiç kimse **saldıramıyordu**. Yani "yağmalanabilir kalsın" diye hayatta bırakılan başkent, iyi niyetli bir gizleme yüzünden dokunulmaz olmuştu. İki kural ayrı ayrı doğru, birleşimi yanlış — ve hiçbir test ikisini birlikte ölçmüyordu | Bir bayrak eklerken sor: *"bu kolonu/tabloyu başka kim OKUYOR?"* `ranking_excluded` bir **gösterim** bayrağıydı ama `rankings` tablosu aynı zamanda bir **oyun mekaniği girdisi**. Sızıntıyı kapatan test tek bir iddiada durmaz: ölçülmesi gereken şey "bayrak doğru mu" değil, **davranışın kendisi** (silinmiş hesaba saldırı gidebiliyor mu) |
| ⭐⭐⭐ Bir kuralı «doğrulanana kadar kapalı» bırakıp **doğrulanabileceği yeri de kapatmak** | Tek cihaz kuralı 2026-08-03'te eksiksiz kuruldu, 13 testle kilitlendi ve **dokuz ay hiç çalışmadı**. Kapı üç koşuldan oluşuyordu: `SINGLE_SESSION_FORCE` (hiçbir yerde tanımlı değil) · `NODE_ENV === 'production'` (dev'de koşulsuz ölüm) · panel anahtarı (varsayılan kapalı, hiç açılmadı). Gerekçe *"canlıda gözle doğrulanmadan açılmamalı"* idi — ama dev'de hiç koşamadığı için doğrulanabileceği **tek yer üretimdi** ve kimse doğrulanmamış bir kuralı üretimde açmadı. Kısır döngü. 2026-08-12 ölçümü: canlıda `settings`te tek bir `session.*` satırı yok, `account_presence` **sıfır satır** | Kapı **tek koşul** olsun ve ortamı bilmesin; veri bütünlüğü kuralının varsayılanı **AÇIK**. ⚠️ Ayrıca testler de yanılttı: üç test *"dev'de uygulanmaz"*ı doğru doğru ölçüyordu — **yeşil test, doğru davranışın değil, yazılmış davranışın kanıtıdır.** Kuralın gerçekten engellediğini ölçen katman (`AuthGuard`) hiç test edilmemişti |
| Mobil istemcide `instanceId`i **her açılışta** üretmek | Web'de doğru olan (sekme başına kimlik) mobilde felaket: uygulama öldürülüp açıldığında yeni kimlik, önceki kimliğin `claimGraceSeconds` (90 sn) boyunca taze duran sahipliğine takılır → oyuncu **kendi hesabına** giremez ve ekranda *"hesabın başka bir cihazda açık"* yazar | Sözleşme: `instanceId` = **çalışan tek kopya**. Web'de sekme (`sessionStorage`), mobilde **kurulum** (kalıcı depo). Mobilde `deviceId` ile aynı olması tamamen doğru |
| ⭐⭐⭐ **Yerel API'ye bakan cihaz doğrulamasını `--release` ile derlemek** | Release APK **yerel API'ye ASLA bağlanamaz** ve hata mesajı bunu hiç söylemez: ekranda yalnız *«Dünya listesi alınamadı»* yazar — yani API çökmüş gibi görünür. Sebep Android 9+: `usesCleartextTraffic` varsayılanı **false** ve `http://127.0.0.1:3002` cleartext. İzin yalnız `android/app/src/debug/AndroidManifest.xml`'de var, `main/`de **yok**. Yıllardır görünmemesinin sebebi `flutter run`ın debug derlemesi. 2026-08-17'de `flutter run` yerine `flutter build apk --release` kullanılınca çıktı ve **üç yanlış teşhise** yol açtı (API kapalı → derleme önbelleği → Cloudflare). ⚠️ APK'nın içindeki adres DOĞRUYDU (`grep -a '127.0.0.1:3002' libapp.so` ile ölçüldü) — hata adreste değil, **taşımaya izin verilmemesindeydi** | Yerel API'ye bakacaksa **daima debug**: `flutter build apk --debug --target-platform android-arm64` (ya da `flutter run`). Release yalnız **https** bir kökle anlamlı: `--release --dart-define=MW_API=https://mobilwar.com`. ⭐ **Başlatıcı: `apps/mobile/tool/dev.ps1`** — tüneli kurar, `/healthz`i yoklar, sonra `flutter run` koşar; bu arıza sınıfını komple kapatır. ⚠️ Ayrıca `adb reverse tcp:3002 tcp:3002` **her USB bağlantısında ve her `flutter install`da sıfırlanır** (2026-08-20'de yine yaşandı: uygulama "yükleniyor"da kaldı, API sapasağlamdı), `flutter install` ise **eski sürümü kaldırıp oturumu siler** (`adb install -r` silmez) |
...
| ⭐⭐ **Flutter kırmızı ekranını kök neden sanmak** | Kırmızı ekran **son** istisnayı yazar, ilkini değil. 2026-08-20'de ittifak metnini kaydederken çıkan ekran `'_dependents.isEmpty': is not true` diyordu; o bir **artçıydı**. Gerçek kök neden konsoldaki İLK bloktu: *"A TextEditingController was used after being disposed"*. Son iddiaya bakılıp yanlış dosyada (`keyboard_guard.dart`) bir gün harcandı ve oraya **çürütülmüş bir neden-sonuç anlatısı** yazıldı | Kök neden için `flutter run` konsolundaki **İLK** `EXCEPTION CAUGHT` bloğu. ⚠️ Dart yığını **logcat'e düşmüyor** — `adb logcat` boş görünür ve "iz yok" sanılır; `flutter run` (ya da `flutter attach`) şart |
| ⭐⭐ Denetleyiciyi route'un `Future`unda atmak (`.whenComplete(c.dispose)`) | Sheet/dialog `Future`u `Navigator.pop` ile **anında** tamamlanır, oysa kapanma animasyonu sürer ve `TextField` hâlâ ağaçtadır. Denetleyici altından çekilince odak değişimi atılmış nesneye dokunur ve çöker. ⚠️ Yalnız **klavye AÇIKKEN** kapatınca: klavye kapalıysa pop sonrası odak değişimi olmaz, kimse dokunmaz → arıza "bazen çalışıyor" görünür ve bir kez yanlışlıkla "düzeldi" diye raporlandı | Denetleyiciyi **onu kullanan parçacık** kursun (`initState`) ve `dispose()`unda atsın. Sınarken alanı **odakla**, öyle kaydet — klavye kapalıyken yapılan deneme hiçbir şey kanıtlamaz |
...
| **Başlangıç yapıları** | Kale · **Baraka** · Çiftlik · Maden **1**, gerisi 0. ⚠️ Baraka 2026-08-09'da 0'a indirilmiş, **2026-08-12'de geri 1'e çekildi** (canlıdaki 29 şehre elle eklendi). ⭐ Fiyat ve PUAN kendiliğinden kayar: `firstPaid` ve `cumulativeBuildingValue` ikisi de `STARTING_BUILDINGS`ten türüyor — baraka sv1 artık ne para ne puan yazar (sv2 = 700/500) |
...
| ⭐ **`MOBIL_UYGULAMA.md`** | **Flutter'a başlamadan ÖNCE aç** (2026-08-13). Hesap kimliğinin ve çoklu hesap **önleme** katmanının niyet belgesi: K0/K1/K2 doğrulama kademeleri · telefon doğrulamasının hangi kapıya konacağı (kayıt DEĞİL, aktarım) · Play Integrity device recall + Apple DeviceCheck (parmak izi değil attestation) · SMS maliyeti ve toll fraud · numaranın HMAC+pepper ile saklanması (⚠️ düz hash işe yaramaz) · silme akışıyla ilişkisi. Tespit katmanı `MOBIWAR_SISTEM_PLANI.md` §9.1'de kalıyor |
| ⭐ **`MOBIL_MIMARI.md`** | **`apps/mobile/` altında iş yapmadan ÖNCE aç** (2026-08-15). Flutter uygulamasının MİMARİSİ ve SIRASI: paket seçimleri · web'den bire bir taşınacak 6 mekanizma · "sözleşme borcu defteri" ve dört sürüklenme kapısı · **test mimarisi (cihaz gerekmiyor — `flutter test` başsız koşar)** · geliştirme döngüsü · faz faz yol haritası. ⭐ §9 **fikir defteri** — akla gelen yeni özellik oraya yazılır. Hesap kimliği tarafı `MOBIL_UYGULAMA.md`'de, derleme/mağaza `DAGITIM.md` §6'da |
