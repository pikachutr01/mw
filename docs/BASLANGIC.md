...
| ⭐⭐ **İstemcide `serverNow()` ile oyun damgası kıyaslamak** | **İKİ KEZ canlıda yaşandı.** `execute_at`/`finish_at`/`startedAt`/`repair_until` hepsi **oyun saatinde**; gerçek saat ondan `clock_offset_ms` kadar ileride (canlıda 196,5 sn). Hata görev SÜRESİ kısaldıkça görünür oluyor: 2026-08-02'de casusluk (120 sn) hep «varıyor» yazdı, 2026-08-07'de asker üretim sayacı kalıcı «sipariş tamamlandı» gösterdi ve sur onarım oranı yanlış okundu | **`gameNow()`**. Yalnız `maintenance_eta` gerçek zamanda → orada `serverNow()`. Bekçi: `apps/web/test/clock.test.ts` |
