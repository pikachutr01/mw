...
| Metinler | ⛔ **`packages/i18n` KURULMUYOR** — karar 2026-08-19'da **geri alındı** | Kullanıcı kararı: metin iki istemcide ayrı ayrı yazılacak. ⚠️ Eski gerekçe (*"tam eşitlikte her metni iki kez yazmak kalıcı vergi olurdu"*) burada **çürütülmüş olarak** duruyor, çünkü kurulum turu başlamış ve vergiyi ÖLÇMÜŞTÜ: iki istemcide birebir aynı duran metin yalnız **202 dizge**, üstelik çoğu tek kelimelik düğme etiketi («Seç», «Üret», «Kıta»). §13.14.2'nin *"metin koda gömülmez"* kuralı böylece **bilinçli** bir ihlale dönüştü — plan değil |
...
`Record<string, unknown>` döndürüyor) · **OpenAPI** · **attestation** ·
...
| Metin | ⛔ **Paylaşılmıyor** — her istemci kendi metnini yazıyor (§1, karar 2026-08-19'da geri alındı). ⚠️ **İstisna, karıştırmayın:** uzun bilgi kutusu metinleri (`BUILDING_INFO` · `UNIT_INFO` · `TECH_INFO`) `ops/facts-to-dart.ts` ile **zaten mobile taşınıyor** ve orada kalıyor. Yani "metin paylaşılmıyor" kuralı onları KAPSAMAZ; o üçünü elle kopyalamak bugün de yanlış |
...
| 2026-08-19 | ⭐⭐ **RAPOR TÜR SÜZGECİ + FAVORİLER** (sunucu + web + mobil). `messages.favorited_at` eklendi (`0051`); uç `?type=` alıyor, iki özel değer var (`all` · `favorites`), gerisi doğrudan `kind` eşleşmesi. ⚠️⚠️ Süzgeç **sunucuda** olmak zorunda — istemcide süzülseydi sunucu 20 satır döndürür, ekran 3'ünü gösterirdi; aynı hata bu uçta `kind` için bir kez yapılmıştı. ⚠️⚠️ Asıl tuzak `total`: süzgeç eklenirken listeyi süzüp toplamı süzmemek en kolay hata ve **sessiz** — 200 raporlu oyuncu «Casusluk» seçince 3 satır görür ama sayfalayıcı «1 / 10» yazar. Süzgeç varken toplam ayrıca sayılıyor, yokken fazladan sorgu hiç atılmıyor. Mutasyonla sınandı. ⚠️ Favori ucu **istenen DURUMU** alıyor (`{favorite: bool}`), toggle DEĞİL: iki istemci aynı raporu açıkken toggle yarışı favoriyi sessizce geri alırdı. ⚠️ Durum gövde ucunda DA dönüyor — rapor bildirim derin bağlantısıyla listeden geçmeden açılabiliyor ve orada liste satırı hiç yok. ⚠️ Arayüz **açılır liste**, çip şeridi değil (kullanıcı aynı gün düzeltti): ilk yazımda yedi çip yatay kayan bir şeritteydi ve seçili olmayanların çoğu ekran dışında kalıyordu — oyuncu neyin var olduğunu görmek için kaydırmak zorundaydı | ✅ yapıldı |
| 2026-08-19 | ⭐⭐ **SAVAŞ RAPORU YENİDEN KURULDU** (iki istemcide de). Sonuç artık **bant** (KAZANDINIZ/KAYBETTİNİZ en üstte, renkli zemin); ordular **gerçek tablo** (Katılan · Kalan · Kayıp + birim resmi). ⚠️ Mobilde *"tablo yok, satır var"* diye yazılı bir karar vardı ve kullanıcı sonucu reddetti (*"anlaması epeyi güç"*); sığdırma sorunu tabloyu bırakarak değil **sütun sayısını azaltarak** çözüldü (web'in altı sütunu → üç). ⚠️ Ganimet üç satırdan **ikiye** indi: «Ganimet» ile «Taşınan» aynı sayıydı, kullanıcı yakaladı. «Kapasiten yetmedi» satırı da kalktı — 2026-08-08'de gerçek bir oyuncu sorusuna cevaptı, artık iki sayının farkı aynı soruyu cevaplıyor. Sunucudaki alanlar duruyor, yalnız çizilmiyor. ⚠️ Ay simgesi artık **tıklanınca açıklıyor** — 2026-08-05'te *"tooltip çıkmasına da gerek yok"* denmişti, aynı kullanıcı kararını tersine çevirdi; eski not korundu ki üçüncü kez tartışılmasın | ✅ yapıldı |
| 2026-08-20 | ⭐⭐ **KIRMIZI EKRAN: `'_dependents.isEmpty': is not true`** (kullanıcı bildirdi, ittifak metni yazarken). Sebep `MwKeyboardGuard`taydı: `View.of(context)` masum bir okuma DEĞİL, **inherited bağımlılık KAYDEDEN** bir arama ve onu `didChangeMetrics` içinde — yani `build`/`didChangeDependencies` dışında, çerçeve ağacı taşırken — çağırmak bağımlılık defterini bozuyor. ⚠️⚠️ Arıza çağrıldığı yerde değil, ilgili inherited öge kaldırılırken patlıyor; teşhisi bu yüzden zor. ⭐ Doğrusu: bağımlılığı yaşam döngüsünün kendi yerinde kur (`didChangeDependencies`), görünümü sakla, gözlemci yalnız **oku**. Kural genel: **inherited arama gözlemci geri çağrısında yapılmaz** | ✅ düzeltildi |
| 2026-08-20 | ⭐⭐ **SAYFA AÇILIRKEN KAHVE→SİYAH RENK SIÇRAMASI** (kullanıcı bildirdi). Sebep bizim kodumuz değil **Flutter'ın varsayılanıydı**: Android'de `ZoomPageTransitionsBuilder` geçiş süresince zemini `colorScheme.surface` ile boyuyor; bizim palette `surface` (#1E1810) sıcak koyu kahve, ekranın gerçek zemini `bg` (#15110C) neredeyse siyah. Her sayfa açılışında yarım saniye kahve, sonra siyah. ⭐ Çözüm geçişi kaldırmak DEĞİL, zeminini `bg`ye bağlamak — animasyon duruyor. ⚠️ Düzeltme **üreteçte** (`design-tokens/build.ts`), `lib/gen/tokens.dart` elle düzenlenmez | ✅ düzeltildi |
| 2026-08-20 | ⭐⭐ **RAPORDAN AÇILAN KOORDİNAT KISA BİR AN PARLIYOR** (iki istemcide de). Slot `?s=` **sorgu parametresi** olarak taşınıyor, yola üçüncü segment eklenmedi: eski bağlantıları kırardı ve slot adresin kimliği değil bir **ipucu**. ⚠️ Kullanıcı iki düzeltme yaptı: (1) iki darbe yerine **tek** darbe, (2) web'de diyar değişince **yeniden parlıyordu** — etkinin bağımlılıklarında `k`/`d` vardı ve başka bir diyarda aynı numaralı slotu parlatıyordu, yani vurgu yanlış yeri işaret ediyordu. Artık `ref` ile «bu `?s=` için parladık» tutuluyor ve elle diyar değişince vurgu sönüyor. ⚠️ Mobilde parlama satırın ZEMİNİNE karışıyor, üstüne katman binmiyor: binen saydam katman yazıyı da boyar ve vurgunun amacı tam da onu okutmak | ✅ yapıldı |
| 2026-08-20 | ⭐ **SEÇENEKLER EKRANI BAŞLADI** — Görünüm (Gece/Gündüz/Sistem) · Hesap · Bildirimler · Tatil modu · Engellenenler. ⚠️ Tema tercihi **önyüklemede** okunuyor (`bootstrap.dart`), sağlayıcıdan async değil: aksi hâlde cihazı gündüz modunda olup «Gece» seçmiş oyuncu her açılışta beyaz bir parlama görürdü — aynı sınıf arıza aynı gün ayrıca düzeltildi. ⛔ **Çıkış yap KONMADI**: web'in kendi kararının tekrarı (*"çıkış zaten sol menüde ve mobilde «Daha» listesinde var, bu ÜÇÜNCÜ kopyaydı"*). ⛔ **Hesap silme bilerek YOK** — Google Play şartı gereği web'de kalıyor (`MOBIL_UYGULAMA.md` §5); yerine adres yazılı. ⛔ Ertelendi: oturumlar · şehir yönetimi · arka plan görseli tercihi | ⏳ v1 hazır |
| 2026-08-19 | ⭐⭐⭐ **GANİMET HESABI EKRANDA KAPANMIYORDU** — kullanıcı canlı raporda yakaladı (*"ortaya çıkandan taşınanı çıkarınca bu sayı çıkmıyor"*). ⚠️ **Hesap hatası YOKTU**, savaş #29 ham kaydıyla doğrulandı: `fromDebris + leftoverDebrisToDefender = debris` kuruşu kuruşuna, `taken` toplamı kapasiteye tam eşit. Arıza **gösterimdeydi**: kalan İKİ kovaya bölünüyor (kasadan sığmayan 785.542 + **enkazdan** sığmayan 6.037.064) ama rapor yalnız birincisini yazıyordu; üçüncü kova görünmediği için fark (6.822.606) hiçbir şeyi açıklamıyordu. ⛔ Bu, aynı turda «Kapasiten yetmedi» satırını silerken kurduğum *"iki sayının farkı aynı soruyu cevaplıyor"* gerekçesini de **çürüttü** — fark iki ayrı sebebi karıştırıyor. Çözüm (kullanıcı kararı): iki satır kalıyor, yanına **info ikonu** geliyor ve tüm döküm tooltip'te. ⭐ Dökümün en değerli satırı *"kapasite önce enkaza harcanır"*: aynı savaşta oyuncu kasadan **sıfır** almıştı çünkü 6,2 milyonluk enkaz kapasitenin tamamını yutmuştu — o kural yazılmadan «kasadan 0» bir hata gibi okunuyor. ⚠️ Dökümün üç kimliği testle kilitli ve mutasyonla sınandı; kimlik bozulursa ekran yine **sessizce** kapanmayan bir hesap gösterirdi | ✅ düzeltildi |
| 2026-08-19 | ⛔ **Tooltip PAKETİ eklenmedi** (kullanıcı izin vermişti). Flutter'ın kendi `Tooltip`i işi zaten yapıyordu; eksik olan tek şey tetikleyiciydi ve `ensureTooltipVisible()` ile tek satır (`MwTapTip`). Tek satırlık bir boşluk için kalıcı bağımlılık, deponun `build_runner`ı reddettiği gerekçenin aynısıyla reddedildi. ⚠️ Uzun basma da çalışmaya devam ediyor — tıklama ek bir yol, mevcut davranışı kaldırmıyor | ⛔ paket yok |
| 2026-08-19 | ⭐⭐ **KLAVYE KAPANINCA ODAK DA KALKIYOR** (`MwKeyboardGuard`, uygulamanın tamamı). Android'de geri tuşuyla klavyeyi kapatmak `TextField`ın odağını almıyordu; kullanıcının bildirdiği pahalı hâli: Baraka'da klavyeyi kapat → bir askere uzun bas → sheet'i kapat → **klavye kendiliğinden geri açılıyor**. ⚠️⚠️ Naif kural («alt boşluk 0 ise odağı bırak») ÇALIŞMAZ: alan odağı aldığı anda klavye henüz açılmamıştır, yani kural o karede odağı geri alır ve hiçbir alana yazı yazılamaz. Bekçi bu yüzden **açık→kapalı GEÇİŞİ** arıyor. ⚠️ Eşik 0 değil 40 px: bazı cihazlarda gezinme çubuğu birkaç piksel bırakıyor ve tam sıfır arayan kural o telefonlarda sessizce hiç çalışmazdı | ✅ yapıldı |
| 2026-08-19 | ⭐ **Sayı kutusunun beyaz çerçevesi ham renk yasağının sessiz ihlaliydi.** `MwAmountInput` ve Dünya'daki diyar alanı **ayrı ayrı** `const OutlineInputBorder()` yazıyordu; rengini vermeyen bir `InputBorder` Material'ın varsayılanına düşüyor ve o varsayılan paletten gelmiyor. Dekorasyon tek yere alındı (`mwFieldDecoration`), kenar artık `border`/`focusRing` token'ından. Kutu 38→32, `MwSmallButton` 34→30 (kullanıcı: satırlarda *"biraz fazla büyük"*) | ✅ yapıldı |
| 2026-08-19 | ⭐ **Ordular şeridinde hücre genişliği artık ekrandan hesaplanıyor** ve kelepçeli. Sabit 76 px dört-beş şehirli oyuncuda ekranı aşıyordu; sondaki şehir yarım göründüğü için "kimisi dar" izlenimi doğuyordu — hücreler aslında eşitti. ⚠️ Düz `Expanded` KULLANILMADI: tek şehirli oyuncuda hücre ekrana yayılır, kale ortada asılı kalırdı (web'de yaşanmış, eski yorumda yazılıydı). Çok şehirde `_kEnAz`ın altına inilmiyor, yatay kaydırma kalıyor — onuncu şehri 30 px'e sıkıştırmak "sığdı" demek olmazdı | ✅ yapıldı |
| 2026-08-19 | ⭐ **Sheet başlıkları kendi zemininde** (`_SheetBaslik`, üç sheet türü de). Renk `panelHeader` — sheet'e özel bir başlık rengi icat etmek, aynı işi yapan iki görsel dil olurdu. ⚠️ `mwSheet`te `titleIsUserText` bayrağı **YOKTU**: Dünya'daki hedef künyesi şehir adını koşulsuz `mwUpper` + Cinzel ile çiziyordu, yani «Mithlond» ekranda «MİTHLOND» görünüyordu — deponun kendi yazılı kuralının ihlali. `mwTallSheet` yüksekliği %88→%94 | ✅ yapıldı |
| 2026-08-19 | ⭐ **Posta kutusunda sayfa boyu artık seçilebilir ve HATIRLANIYOR** (10/20/50, cihazda). ⚠️ Mobildeki *"sayfa boyu SABİT ve seçici yok, telefonda açılır liste masaüstü mobilyası"* kararı kullanıcı isteğiyle **geri alındı**; varsayılan da 20'den web'in 10'una indi ki iki istemci aynı yerden başlasın. ⚠️⚠️ Diskten okunan değer **doğrulanıyor**: sunucu `limit`i 1..100'e kıskaçlıyor, dolayısıyla bozuk bir `5000` kaydı listeyi 100 satır getirip ekranı «1 / 1» yazdırırdı — sayfalayıcı sessizce yalan söyler ve kalan mesajlara ulaşılamazdı (`normalizeMessagePageSize`, mutasyonla sınandı). ⚠️ Tercih **sağlayıcıdan** okunuyor, ekranın `initState`inden değil: aksi hâlde her açılışta biri varsayılanla biri gerçek boyla **iki** posta kutusu isteği giderdi | ✅ yapıldı |
| 2026-08-19 | ⛔⛔ **SALDIRI VE CASUSLUK ARTIK HİÇ PUSH ATMIYOR** (kullanıcı). Dört bildirim (`attack` kovası): gelen saldırı · gelen casus kuşu · savaş sonucu · casusluk raporu. ⭐ **Toast KESİLMEDİ** — oyun açıkken hepsi görünmeye devam ediyor; kesilen yalnız kilitli telefona düşen haber. Mekanizma zaten vardı (`push: false`) ve `deliverOne`daki tek dallanma bunu yapısal olarak garanti ediyor: kapı toast dalının ALTINDA. ⚠️⚠️ Seçenekler'den anahtar kaldırılınca bir TUZAK doğdu: hesaplarda duran eski `{"attack": false}` kayıtları toast'ı **sonsuza kadar** susturur ve geri açmanın yolu olmazdı (arayüzde anahtar yok, API şeması reddediyor). Çözüm `NOTIFY_CONFIGURABLE`: ayarlanamayan kategoride kayıtlı değere HİÇ bakılmıyor, böylece göç de gerekmedi. Testle kilitli | ✅ yapıldı |
| 2026-08-19 | ⭐ **Sıralama künyesinde başkent koordinatı** (iki istemcide de). ⚠️⚠️ Koordinat sıralama SORGUSUNA eklenmedi, künye açılınca **oyuncu başına tek istek** gidiyor: listeye koymak tek istekte bir sayfa dolusu başkent dağıtırdı ve bu depo o farka duyarlı (kahramanın `xp` alanı, ekranda görünmediği hâlde ağ sekmesinden okunabildiği için sorgudan çıkarılmıştı). ⭐ Kullanılan uç «Dünyada bul»un zaten çağırdığı arama ucu ve yapısı gereği `is_capital` ile sınırlı — *"sadece başkent"* şartı sunucuda hâlihazırda yazılı. Çekilen sonuç düğmede de kullanılıyor, yani düğme artık çoğu zaman hiç istek atmıyor | ✅ yapıldı |
| 2026-08-19 | ⛔⛔ **`packages/i18n` KARARI GERİ ALINDI** (kullanıcı kararı). Paket kurulmuş, tarayıcı ve sürüklenme kapısı yazılmış, ilk ölçüm alınmış ve **tamamı silinmişti**; istemcilere hiç dokunulmamıştı, o yüzden geri alma tek dizin silmekti. ⭐ Ölçüm yine de kayda değer, çünkü kurulum gerekçesini **çürütüyor**: web'de 88, mobilde 84 dosya tarandığında iki istemcide **birebir aynı** duran metin yalnız **202 dizge** çıktı ve çoğu tek kelime («Seç», «Üret», «Kıta», «Yukarı»). Vergi varsayıldığı kadar büyük değildi. ⚠️ Sonuç: metin **iki istemcide ayrı ayrı** yazılıyor; ayrışırsa bunu yakalayacak bir kapı YOK, gözle görülecek | ⛔ vazgeçildi |
| 2026-08-19 | ⚠️ **Aksanı düşmüş Türkçe üç metin oyuncuya görünüyordu** — `providers.dart`ta «Ittifak okunamadi.» · «Ittifak kunyesi okunamadi.» · «Ittifak sohbeti acilamadi.». Üçü de İttifak turundan kalma; aynı dosyadaki **on bir kardeşi** doğru yazılmıştı, yani kural değil dikkat kaçağıydı. ⭐ Avlama yöntemi kayda değer: elle "yanlış yazımlar listesi" tutmak yerine deponun KENDİ doğru yazımları toplanıp aksanları silindi, sonra o aksansız biçimler metinlerde arandı — kendi kendini kalibre eden bir tarama. ⚠️ Yalancı eşleşmeler bilerek elendi: `$gun`/`$sec` gibi **değişken adları**, «tur» (≠ «tür»), «oldu» (≠ «öldü»), `ornek@eposta.com`. ⚠️ Sunucuda ASCII Türkçe **log** satırları duruyor (`worker.ts` · `auth.service.ts` · `mission.repository.ts`); oyuncuya görünmedikleri için dokunulmadı | ✅ düzeltildi |
