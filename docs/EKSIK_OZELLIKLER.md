...
| Konu | Kanıt | Durum |
| :-- | :-- | :-- |
| ⭐ **Kahraman mağaraya saklanabiliyor** | `k.java` dize tablosu `231+durum`: **Mağarada · Mağaraya Giriyor · Mağaradan Çıkıyor** (kod 3-4-5); mağara ekranının durum süzgeci `i.java:844-852` | ✅ **UYGULANDI 2026-08-11.** `heroes.status='in_cave'` (yeni tablo yok), alan **5** (ölçülmüş), asker+kahraman **tek modal**. Aynı turda askerlere de **rezervasyon** kuralı geldi: mağara emrindeki birlik başka göreve gönderilemez. Tasarım `MOBIWAR_SISTEM_PLANI.md` §13.20.2c/§13.20.6, röntgen `JAVA_ROENTGEN.md` §6.2 |
...
| ~~**i18n dosyaları**~~ | `MOBIWAR_SISTEM_PLANI.md:1990` kuralı: *"metin asla koda gömülmez → i18n/tr.json"* | ⛔ **2026-08-19'da VAZGEÇİLDİ** (kullanıcı kararı). Kural artık **bilerek** uygulanmıyor: metin web'de JSX içinde, mobilde Dart içinde, ayrı ayrı yazılıyor. Ölçüm ve gerekçe `MOBIL_MIMARI.md` §1'de — iki istemcide birebir ortak metin yalnız 202 dizge çıktı, çoğu tek kelime. ⚠️ İstisna: uzun bilgi kutusu metinleri (`BUILDING_INFO` · `UNIT_INFO` · `TECH_INFO`) `ops/facts-to-dart.ts` ile mobile taşınmaya **devam ediyor** |
| **Kuşatma bonusu** (Mancınık/Ogre → sur) | §13.5 Faz 3 | Kodlanmadı |
| ~~**Gece savaşı over-kill artığı**~~ | — | ✅ **2026-08-11'de KAPANDI.** "%15" iddiası 2026-07-22 öncesindendi. Mekanizma 2026-07-31'de Ghidra ile doğrulandı (`FUN_00412624` = Can + BüyüCan) ve taşıma kapasitesinin boşuna çarpılması düzeltildi. 2026-08-11'de **belirleyici set** koşuldu (`GECE_GORUS_TESTLERI.md`, 9 grup / 60+ hücre, **9/9 tuttu**): gece görüşü **tamamen ofansif**, eğri **logaritmik değil**, motorda eksik kanal **yok** ve ⭐ çarpanın **yedi değeri de ölçümden geri çözüldü** (taban `0,7` → 0,6976-0,7022). Artık disassembly okumasına değil ölçüme dayanıyor |
